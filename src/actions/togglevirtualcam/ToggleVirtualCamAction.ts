...
export class ToggleVirtualCamAction extends AbstractStatefulRequestAction<ActionSettings, 'VirtualcamStateChanged'> {
	...
	getPayloadFromSettings(settings: Record<string, never> | Partial<ActionSettings>, desiredState?: number | undefined): SingleRequestPayload<'StartVirtualCam' | 'StopVirtualCam' | 'ToggleVirtualCam'> {
		if (desiredState === 0) {
